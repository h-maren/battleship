...
const compPlayerGameSquares = (function () {
  ...
  let compPlacementGameboard = compPlayerGameboard.querySelector(
    ".ship-placement-gameboard"
  );
  let compPlacementSquares =
    compPlacementGameboard.querySelectorAll(".game-square");
...
function compPlayerPlaceShips(ship) {
  ...
  let validCheck = compPlayer.gameboard.placeShip(
    ship,
    compCoord,
    shipDirection
  );
...
const getShipCoords = function (shipType, shipDirection, shipCoords) {
  ...
  if (
    ...
    let validCheck = realPlayer.gameboard.placeShip(
      newShip,
      shipCoords,
      shipDirection
    );
...
const playRound = function (e) {
  ...
  let compGameboardMA = compGameboard.querySelector(
    ".missed-attacks-gameboard"
  );
  let compSquare = compGameboardMA.querySelector(
    `[data-xcoord="${xCompCoord}"][data-ycoord="${yCompCoord}"]`
  );
  ...
  if (compSquare.classList.contains("hit")) {
    let shipCheck =
      realPlayer.gameboard.shipPlacement[xCompCoord][yCompCoord].isSunk();
    if (shipCheck == true) {
      ...
      let humanSunkenShipList = document.querySelector(
        ".human-sunken-ships-list"
      );
      let sunkenShipListItem = humanSunkenShipList.querySelector(
        `li.${realPlayer.gameboard.shipPlacement[xcoord][ycoord].type}-sunk`
      );
      sunkenShipListItem.classList.add("sunken");
      if (allRPShipsSunk) {
...
function convertCoord(xcoord, ycoord) {
  let coord =
    String.fromCharCode(Number(xcoord) + 65) + String(Number(ycoord) + 1);
  return coord;
}
